...
# -------------------------------------------------------------
# 2. Resoudre les stores dupliques
#    Garder store/ a la racine (utilise par Providers.jsx @/store)
#    Supprimer app/store/ (doublon)
# -------------------------------------------------------------
Write-Host "`n[ 2 ] Suppression du store duplique (app/store/)..." -ForegroundColor Magenta

$appStore = Join-Path $root "app\store"
if (Test-Path $appStore) {
    Remove-Item $appStore -Recurse -Force
    Log "Supprime : app/store/ (doublon - garder store/ a la racine)"
} else { SKIP "app/store/ deja absent" }

# -------------------------------------------------------------
# 3. Resoudre lib/api duplique
#    Garder lib/api.jsx a la racine (import @/lib/api)
...
if (Test-Path $appLib) {
    ...
    if ($items.Count -eq 0) {
        ...
        Log "Supprime : app/lib/ (vide)"
    }
}

# -------------------------------------------------------------
# 4. S'assurer que lib/serverFetch.js est bien a la racine lib/
#    (les pages importent @/lib/serverFetch)
# -------------------------------------------------------------
Write-Host "`n[ 4 ] Verification lib/serverFetch.js..." -ForegroundColor Magenta

$sfPath = Join-Path $root "lib\serverFetch.js"
if (Test-Path $sfPath) {
    OK "lib/serverFetch.js present"
} else {
    Log "Creation de lib/serverFetch.js manquant"
    $sfContent = @'
/**
...
OK "Corrige $countTo fichiers (Link to= -> href=)"

# -------------------------------------------------------------
# 6. Supprimer imports useNavigate, useLocation, useParams (react-router)
#    et remplacer par hooks Next.js
# -------------------------------------------------------------
Write-Host "`n[ 6 ] Nettoyage imports react-router-dom residuels..." -ForegroundColor Magenta
...
foreach ($file in $allFiles) {
    ...
    Set-Content -Path $file.FullName -Value $newContent -Encoding UTF8
    $countRouter++
    Log "react-router-dom retire : $($file.Name)"
}
}
OK "Corrige $countRouter fichiers (imports react-router-dom)"
...
if (Test-Path $contactPath) {
    ...
    OK "ContactClient.jsx nettoye (Helmet supprime)"
}
...
if (Test-Path $structuredPath) {
    ...
    OK "StructuredData.jsx nettoye (import Helmet supprime)"
}
...
if (Test-Path $homeClientPath) {
    ...
    OK "HomeClient.jsx selecteur Redux corrige"
}
...
if (Test-Path $createRMAPath) {
    ...
    OK "CreateRMAClient.jsx corrige"
}
...
if (Test-Path $orderDetailsPath) {
    ...
    # useParams en Next.js App Router ne peut pas etre utilise dans un Client Component de cette facon
    # Le orderId est passe en prop depuis la page
    ...
    OK "OrderDetailsClient.jsx corrige"
}
...
if (Test-Path $articleDetailPath) {
    ...
    OK "ArticleDetailClient.jsx corrige"
}
...
#     (les thunks async du app/store/slices/wishlistSlice.js sont supprimes avec app/store)
# -------------------------------------------------------------
Write-Host "`n[ 20 ] Verification wishlistSlice.js..." -ForegroundColor Magenta

$wishlistPath = Join-Path $root "store\slices\wishlistSlice.js"
if (Test-Path $wishlistPath) {
    OK "store/slices/wishlistSlice.js present et correct"
}

# -------------------------------------------------------------
# 21. Corriger les imports @/store dans les composants
#     qui pointaient vers app/store (supprime) -> maintenant store/
#     Next.js resout @/ depuis la racine donc @/store = store/  [OK]
# -------------------------------------------------------------
Write-Host "`n[ 21 ] Verification coherence imports @/store..." -ForegroundColor Magenta
...
# Tout devrait resoudre vers store/ a la racine -> OK
OK "Imports @/store correctement resolus vers store/"
...
if (Test-Path $dashPath) {
    ...
    OK "DashboardClient.jsx corrige"
}
...
if (Test-Path $footerPath) {
    ...
    OK "Footer.jsx corrige"
}

# -------------------------------------------------------------
# 26. Scan final - detecter les Link to= restants
# -------------------------------------------------------------
...
if ($remaining.Count -gt 0) {
    Log "Corrige Link to->href dans : $($remaining -join ', ')"
} else {
...
if ($rrFiles.Count -gt 0) {
    Write-Host "  [!] Fichiers avec react-router-dom encore present :" -ForegroundColor Red
...
# -------------------------------------------------------------
# 28. Verifier que tailwind.config.js inclut le dossier store/
# -------------------------------------------------------------
Write-Host "`n[ 28 ] Verification tailwind.config.js content paths..." -ForegroundColor Magenta
...
if (Test-Path $tailwindPath) {
    ...
    if ($tw -notmatch "store") {
        ...
        Log "tailwind.config.js : ajoute store/ dans content"
...
Write-Host "  2. npm run build   (verifier les erreurs restantes)" -ForegroundColor White
