  ...
  return (
    <div className="bg-white dark:bg-surface-800 rounded-2xl shadow-card p-6 sticky top-24">
      ...
      <div className="border-t border-surface-100 dark:border-surface-700 pt-4 space-y-3">
        ...
        {/* Livraison */}
        <div className="flex items-center justify-between text-sm">
          <span className="text-surface-600 dark:text-surface-400">Livraison</span>
          <span className={`font-semibold ${deliveryFee === 0 ? "text-green-600 dark:text-green-400" : "text-surface-900 dark:text-white"}`}>
            {deliveryFee === 0 ? "Gratuit" : `${deliveryFee.toLocaleString()} FCFA`}
          </span>
        </div>

        {/* Total */}
        <div className="border-t border-surface-200 dark:border-surface-600 pt-3 mt-2">
      ...
      {/* Note sécurité */}
      <div className="mt-5 flex items-center gap-2 text-xs text-surface-500 dark:text-surface-400 bg-surface-50 dark:bg-surface-700/50 p-3 rounded-lg">
